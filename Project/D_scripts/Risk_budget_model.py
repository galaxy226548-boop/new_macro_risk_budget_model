...
RUN_NAME = "risk_budget_model_unlevered"
RUN_TS = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
OUTPUT_DIR = os.path.join(PROJECT_DIR, "C_output", "Risk_budget")
...
# 风险预算来源设置：
# - "manual"：后续步骤中手动输入股/债/商风险预算
# - "file"：后续步骤中从 events/trades 文件生成风险预算
RISK_BUDGET_SOURCE = "file"
CLASS_BUDGET_ORDER = ["股", "债", "商"]
DEFAULT_MANUAL_CLASS_BUDGET = {"股": 0.4, "债": 0.4, "商": 0.2}

# 文件模式路径：参照 Risk_budget_model_reference.py 的
# Final_Results_20260223_235650\events_all.xlsx / trades_OOS.xlsx
risk_budget_signal_dir = os.path.join(PROJECT_DIR, "C_output", "Final_Results_20260223_235650")
events_docu = os.path.join(risk_budget_signal_dir, "events_all.xlsx")
trades_docu = os.path.join(risk_budget_signal_dir, "trades_OOS.xlsx")


def _resolve_risk_budget_source(default_source: str) -> str:
    valid_sources = {"manual", "file"}
    if default_source not in valid_sources:
        raise ValueError(f"RISK_BUDGET_SOURCE 只能是 {sorted(valid_sources)}，当前为：{default_source}")

    if sys.stdin is None or not sys.stdin.isatty():
        print("ℹ️  当前运行环境不可交互，风险预算来源自动使用 manual 模式")
        return "manual"

    print("\n" + "=" * 52)
    print("请选择风险预算来源：")
    print("  [1] manual：手动输入股/债/商风险预算")
    print("  [2] file  ：从 events/trades 文件生成风险预算")
    print("=" * 52)

    while True:
        try:
            user_input = input(f"请输入选择 [1/2，直接回车默认 {default_source}] >>> ").strip()
        except EOFError:
            print("⚠️  当前运行环境无法读取输入，风险预算来源自动使用 manual 模式")
            return "manual"
...
source = _resolve_risk_budget_source(RISK_BUDGET_SOURCE)
print(f"✅ 风险预算来源：{source}")
if source == "file":
    print("⚠️  file 模式将在后续步骤实现，本步仅保留入口占位；当前主回测仍使用原风险平价逻辑。")
    print("   events:", events_docu)
    print("   trades:", trades_docu)
...
def _validate_asset_class_map(assets: list, asset_class_map: dict) -> dict:
    missing = [a for a in assets if a not in asset_class_map]
    if missing:
        raise ValueError(f"ASSET_CLASS_MAP 缺少以下资产分类：{missing}")
    return {a: asset_class_map[a] for a in assets}

def _normalize_class_budget(values, label: str = "class budget") -> pd.Series:
    if isinstance(values, dict):
        arr = pd.Series(values, dtype="float64").reindex(CLASS_BUDGET_ORDER)
    else:
        arr = pd.Series(list(values), index=CLASS_BUDGET_ORDER, dtype="float64")
    if arr.isna().any():
        raise ValueError(f"{label} 必须包含股/债/商三类预算")
    if (arr < 0).any():
        raise ValueError(f"{label} 不能包含负数：{arr.to_dict()}")
    total = float(arr.sum())
    if (not np.isfinite(total)) or total <= 0:
        raise ValueError(f"{label} 的合计必须大于 0：{arr.to_dict()}")
    return arr / total

def _parse_manual_class_budget(text: str) -> pd.Series:
    parts = text.replace(",", " ").replace("，", " ").replace(";", " ").replace("；", " ").split()
    if len(parts) != 3:
        raise ValueError("请输入 3 个数字，顺序为：股 债 商，例如 40 40 20 或 0.4 0.4 0.2")
    try:
        values = [float(x) for x in parts]
    except ValueError as exc:
        raise ValueError("风险预算只能包含数字，例如 40 40 20 或 0.4 0.4 0.2") from exc
    return _normalize_class_budget(values, label="manual class budget")

def _get_manual_class_budget() -> pd.Series:
    default_budget = _normalize_class_budget(DEFAULT_MANUAL_CLASS_BUDGET, label="DEFAULT_MANUAL_CLASS_BUDGET")
    if sys.stdin is None or not sys.stdin.isatty():
        print("ℹ️  当前运行环境不可交互，使用 DEFAULT_MANUAL_CLASS_BUDGET")
        return default_budget

    print("\n" + "=" * 52)
    print("请输入股/债/商的初始风险预算分数：")
    print("  示例：40 40 20 或 0.4 0.4 0.2")
    print("  直接回车使用 DEFAULT_MANUAL_CLASS_BUDGET")
    print("=" * 52)

    while True:
        user_input = input("请输入 股 债 商 >>> ").strip()
        if not user_input:
            return default_budget
        try:
            return _parse_manual_class_budget(user_input)
        except ValueError as exc:
            print(f"⚠️  {exc}")

def _filter_class_budget_to_selected_assets(class_budget: pd.Series, asset_class_by_asset: dict) -> pd.Series:
    active_classes = [c for c in CLASS_BUDGET_ORDER if c in set(asset_class_by_asset.values())]
    if not active_classes:
        raise ValueError("selected assets 中没有任何可用的大类资产")
    filtered = class_budget.reindex(active_classes).fillna(0.0)
    total = float(filtered.sum())
    if (not np.isfinite(total)) or total <= 0:
        raise ValueError(f"已选资产对应大类的预算合计必须大于 0：{filtered.to_dict()}")
    return filtered / total

_selected = _select_assets(ret_sub.columns.tolist(), SELECTED_ASSETS)
asset_class_by_asset = _validate_asset_class_map(_selected, ASSET_CLASS_MAP)
...
def risk_budget_weights_from_window(
    ...
    """
    输入：
    - ret_window：窗口内日对数收益，T×N
    - target_risk_budget：目标风险贡献比例，长度为 N，sum=1
    - w0：上一期持仓权重初值

    输出：满足目标风险预算的持仓权重 w（N,），约束 sum(w)=1, 0<=w<=1。
    target_risk_budget 是目标风险贡献比例，不是持仓权重。
    """
    n_local = int(ret_window.shape[1])
    if n_local <= 0:
        return np.array([])
    ...
    def _clean_target_risk_budget(values: np.ndarray) -> np.ndarray:
        target_equal = np.ones(n_local, dtype="float64") / n_local
        try:
            target = np.asarray(values, dtype="float64").copy()
        except Exception:
            return target_equal
        if target.ndim != 1 or target.shape[0] != n_local:
            return target_equal
    ...
    w_prev = _clean_w(w0)
    target_risk_budget = _clean_target_risk_budget(target_risk_budget)
    ...
    def obj_w(w):
        ...
        MRC = Sw / R
        TRC = w * MRC
        target_trc = target_risk_budget * R
        return float(np.sum((TRC - target_trc) ** 2))
    ...
    w = w / s
    if w.shape[0] != n_local or (not np.isfinite(w).all()):
        return w_prev

    return w

def _sanity_check_risk_budget_solver() -> None:
    for t_reb in rebalance_dates:
        ...
        n_local = int(ret_window.shape[1])
        target_risk_budget_equal = np.ones(n_local, dtype="float64") / n_local
        w_rp = risk_parity_weights_from_window(ret_window, w0=None)
        w_rb_equal = risk_budget_weights_from_window(ret_window, target_risk_budget_equal, w0=None)
        max_abs_diff = float(np.max(np.abs(w_rp - w_rb_equal)))
        print(
            "risk_budget_weights_from_window sanity check max_abs_diff "
            f"(equal target vs risk parity, {pd.Timestamp(t_reb).date()}): {max_abs_diff:.12g}"
        )
        return

    print("⚠️  risk_budget_weights_from_window sanity check skipped: no ret_window with enough samples")

_sanity_check_risk_budget_solver()

# ----- Manual 风险预算目标表（仅生成目标风险预算，不接入主回测）-----
class_budget_manual = None
risk_budget_at_rebalance = None
risk_budget_daily_target = None

def _build_manual_risk_budget_targets(
    returns: pd.DataFrame,
    rebalance_dates: pd.Series,
    assets: list,
    asset_class_by_asset: dict,
    class_budget_manual: pd.Series,
    lookback_months: int,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    assets_by_class = {
        cls: [a for a in assets if asset_class_by_asset[a] == cls]
        for cls in class_budget_manual.index
    }
    prev_class_weights: dict[str, np.ndarray] = {}
    rows = []

    for t_reb in rebalance_dates:
        t_start = t_reb - pd.DateOffset(months=lookback_months)
        ret_window = returns.loc[(returns.index >= t_start) & (returns.index <= t_reb), assets]
        if ret_window.empty:
            continue

        rw_t = pd.Series(0.0, index=assets, name=t_reb, dtype="float64")
        for cls, class_assets in assets_by_class.items():
            if not class_assets:
                continue

            class_budget = float(class_budget_manual.loc[cls])
            if len(class_assets) == 1:
                class_inner_weights = np.array([1.0], dtype="float64")
            else:
                ret_window_cls = ret_window.loc[:, class_assets]
                w0 = prev_class_weights.get(cls)
                class_inner_weights = risk_parity_weights_from_window(ret_window_cls, w0=w0)
                prev_class_weights[cls] = class_inner_weights

            rw_t.loc[class_assets] = class_budget * class_inner_weights

        total = float(rw_t.sum())
        if (not np.isfinite(total)) or total <= 0:
            raise ValueError(f"{t_reb} 生成的底层资产目标风险预算合计无效：{total}")
        rows.append(rw_t / total)

    if not rows:
        raise ValueError("manual 模式没有生成任何调仓日目标风险预算")

    risk_budget_at_rebalance = pd.DataFrame(rows, columns=assets)
    risk_budget_at_rebalance.index.name = "RebalanceDate"

    risk_budget_daily_target = (
        risk_budget_at_rebalance
        .reindex(returns.index)
        .ffill()
        .bfill()
        .loc[:, assets]
    )
    risk_budget_daily_target.index.name = "Trddt"

    risk_budget_at_rebalance = risk_budget_at_rebalance.div(risk_budget_at_rebalance.sum(axis=1), axis=0)
    risk_budget_daily_target = risk_budget_daily_target.div(risk_budget_daily_target.sum(axis=1), axis=0)
    return risk_budget_at_rebalance, risk_budget_daily_target

if source == "manual":
    # 后续信号微调将在这里调整股/债/商大类预算比例。
    class_budget_manual = _filter_class_budget_to_selected_assets(
        _get_manual_class_budget(),
        asset_class_by_asset,
    )
    risk_budget_at_rebalance, risk_budget_daily_target = _build_manual_risk_budget_targets(
        returns=ret_sub,
        rebalance_dates=rebalance_dates,
        assets=assets,
        asset_class_by_asset=asset_class_by_asset,
        class_budget_manual=class_budget_manual,
        lookback_months=LOOKBACK_MONTHS,
    )

    print("=== class_budget_manual ===")
    print(class_budget_manual)
    print("=== risk_budget_at_rebalance.head()（目标风险预算，不是实际持仓权重） ===")
    print(risk_budget_at_rebalance.head())
    print("=== risk_budget_daily_target.head()（目标风险预算，不是实际持仓权重） ===")
    print(risk_budget_daily_target.head())
    reb_sum = risk_budget_at_rebalance.sum(axis=1)
    daily_sum = risk_budget_daily_target.sum(axis=1)
    print(
        "risk_budget_at_rebalance row sum min/max:",
        float(reb_sum.min()),
        float(reb_sum.max()),
    )
    print(
        "risk_budget_daily_target row sum min/max:",
        float(daily_sum.min()),
        float(daily_sum.max()),
    )
else:
    print("ℹ️  file 模式目标风险预算生成将在后续步骤实现；当前不生成 risk_budget_at_rebalance / risk_budget_daily_target。")
...
# ✅ 用于风险预算优化器的初始权重：首期等权，之后沿用上一期目标权重
...
for i, t_reb in enumerate(rebalance_dates):
    ...
    # ② 在 t_reb 收盘后，先取当期目标风险预算，再反推出目标持仓权重
    if risk_budget_at_rebalance is None:
        raise NotImplementedError("file 模式尚未生成 risk_budget_at_rebalance；当前主回测请先使用 manual 模式")
    if t_reb not in risk_budget_at_rebalance.index:
        raise ValueError(f"risk_budget_at_rebalance 缺少主回测调仓日：{t_reb}")

    target_risk_budget = risk_budget_at_rebalance.loc[t_reb, assets].values
    w_t = risk_budget_weights_from_window(ret_window, target_risk_budget, w0=w_prev)
...
weights_row_sum = weights_df.sum(axis=1)
print("weights_df row sum min/max:", float(weights_row_sum.min()), float(weights_row_sum.max()))
if risk_budget_at_rebalance is not None:
    missing_budget_dates = weights_df.index.difference(risk_budget_at_rebalance.index)
    if len(missing_budget_dates) > 0:
        raise ValueError(f"risk_budget_at_rebalance 与主回测实际调仓日未对齐，缺失：{missing_budget_dates.tolist()}")
    print("risk_budget_at_rebalance aligned with main rebalance dates:", len(weights_df.index))
...
print("daily_weights_df source: buy & hold drifted actual holding weights")
...
plt.title("Risk Budget Strategy Cumulative NAV")
plt.xlabel("Date")
plt.ylabel("NAV")
plt.grid(True)
...
print("risk_contribution_df generated:", risk_contribution_df.shape)

plt.figure(figsize=(12, 5))
for asset in assets:
    plt.plot(daily_weights_df.index, daily_weights_df[asset], label=asset, linewidth=1.4)
plt.title("Risk Budget Daily Asset Allocation Weights")
plt.xlabel("Date")
plt.ylabel("Weight")
...
ax.set_title("Risk Budget Annual Average Asset Allocation Weights")
ax.set_xlabel("Year")
...
ax.set_title("Risk Budget Annual Average Asset Risk Contribution")
ax.set_xlabel("Year")
...
plt.figure(figsize=(10, 4))
plt.plot(rp_nav.index, rp_nav.values, label="Risk Budget")
